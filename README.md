...
- **新增**：Web 概览页显示**近 5 分钟**的 token 速度——顶部为所有实时会话的 **output 速度合计**，下方逐会话以卡片式列出（每会话信息区三行）：第一行 **客户端 / 请求**，第二行 **协议 / 上下文 / 最后活动**，第三行 **标识**；右侧单行 **输出 / 预输入 / 历史最佳 / 均值**（最多 30 个会话）。输出速度按阈值**分级配色**（<27 红 / 27-53 橙红 / 53-70 绿 / ≥70 紫），顶部合计与 `tok/s` 同步变色。页面自动轮询刷新。
  - **客户端** = 访问本服务（bc）的**真实来源 `IP:端口`**（非后端引擎地址）。
  - **列表** = 仅保留**最近 5 分钟内有活动**的会话，按最后活动时间倒序，最多 30 条；超 5 分钟无活动自动移出列表。
  - **异常速度护栏** = 生成耗时 < 125ms 的采样不计入 output 速度，避免空/瞬间响应产生 4W+ 之类尖峰被「历史最佳」永久放大。
