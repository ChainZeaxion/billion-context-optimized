...
- **新增**：Web 概览页显示**近 5 分钟**的 token 速度——顶部为所有实时会话的 **output 速度合计**，下方逐会话以卡片式列出（每会话两行）：第一行 **客户端地址 / 协议 / 请求次数 / 上下文 / 最后活动**，第二行 **标识 / 会话**，右侧固定单行 **output 速度 / prefill 速度 / 历史最佳 / 均值**（最多 30 个会话）。近 5 分钟无实时数据的会话显示历史最佳与均值；有数据则显示、5 分钟无数据自动隐藏，页面自动轮询刷新。
