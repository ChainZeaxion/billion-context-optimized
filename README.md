...
- **新增**：Web 概览页显示**近 5 分钟**的 token 速度——顶部为所有实时会话的 **output 速度合计**，下方逐会话以卡片式列出（每会话信息区三行）：第一行 **客户端 / 请求**，第二行 **协议 / 上下文 / 最后活动**，第三行 **标识**；右侧单行 **输出 / 预输入 / 历史最佳 / 均值**（最多 30 个会话）。输出速度按阈值**分级配色**（<27 红 / 27-53 橙红 / 53-70 绿 / ≥70 紫），顶部合计与 `tok/s` 同步变色。近 5 分钟无实时数据的会话显示历史最佳与均值；有数据则显示、5 分钟无数据自动隐藏，页面自动轮询刷新。
